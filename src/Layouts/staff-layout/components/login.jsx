...
export default function Login() {
  ...
  const handleSubmit = (data) => {
    if(data){
      ...
      postRequest("/api/staff-login",payload).then((res)=>{
        if(res.data.status === true && res.data.code ===201){
          setLoading(true);
          sessionStorage.setItem("staffAuth",JSON.stringify(res.data.token));
          sessionStorage.setItem("employee",JSON.stringify(res.data.emp));
            setTimeout(() => {
              setLoading(false);
                ...
                <Form>
                  <div className="form-group mt-2 mb-2">
                    <label htmlFor="email" className="mb-1 mt-1">
                      Designation Email <span className="text-danger">*</span>
                    </label>
                    <Field
                      type="text"
                      // + }
